...
def addpersontraveling(request,id_riquest):
    ...
    form = PersonTravelingForm()

    if request.method == 'POST':
        form = PersonTravelingForm(request.POST)
        if form.is_valid():
            ...
            return redirect('travel:detallutravelrequesttab', id = encript_id_riquest, tab = 'Person')
        else:
            messages.error(request, 'There was an error. Please correct the form.')  # Error message
            return redirect('travel:addpersontraveling', id_riquest = encript_id_riquest)

    context = {
        "form" : form,
        "pajina_travel" : "active",
            }
    return render(request, 'travel/addpersontraveling.html',context)



def editpersontraveling(request,id_item):


    dec_id_item = decrypt_id(id_item)
    itemedit = PersonTraveling.objects.get(id=dec_id_item)

    id_request = encrypt_id(str(itemedit.travel_autorization.id))

    form = PersonTravelingForm(instance = itemedit)

    if request.method == 'POST':
        form = PersonTravelingForm(request.POST, instance = itemedit)
        if form.is_valid():
            instance = form.save(commit=False)
            instance.save()
            messages.success(request, 'Request created successfully.')  # Success message
            ...
            messages.error(request, 'There was an error. Please correct the form.')  # Error message
            return redirect('travel:addpersontraveling', id_item = id_item)
...
def apagapersontraveling(request, id_item):
    id_item = decrypt_id(id_item)
    itemapaga = PersonTraveling.objects.get(id=id_item)
