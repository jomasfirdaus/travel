...
def addroutetraveling(request,id_riquest):
    ...
    form = RouteTravelForm()

    if request.method == 'POST':
        form = RouteTravelForm(request.POST)
        if form.is_valid():
    ...
    return render(request, 'travel/addroutetraveling.html',context)



def editroutetraveling(request,id_item):


    dec_id_item = decrypt_id(id_item)
    itemedit = RouteTravel.objects.get(id=dec_id_item)

    id_request = encrypt_id(str(itemedit.travel_autorization.id))

    form = RouteTravelForm(instance = itemedit)

    if request.method == 'POST':
        form = RouteTravelForm(request.POST, instance = itemedit)
        if form.is_valid():
            instance = form.save(commit=False)
            instance.save()
            messages.success(request, 'Request created successfully.')  # Success message
            ...
            messages.error(request, 'There was an error. Please correct the form.')  # Error message
            return redirect('travel:editroutetravel', id_item = id_item)
...
def apagaroutetraveling(request, id_item):
    id_item = decrypt_id(id_item)
    itemapaga = RouteTravel.objects.get(id=id_item)
